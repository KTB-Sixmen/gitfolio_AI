...
def process_repository(repo_url, githubID, githubName):
    try:
        # 1. 코드 파일 클론 및 요약 생성
        final_code_summary = process_code_files(repo_url, githubID)

        # 2. PR 요약 생성
        final_pr_summary = process_pr_text(repo_url, githubID)

        # 3. 커밋 diff 요약 생성
        final_commit_summary = process_commit_diffs(repo_url, githubID, githubName)

        # 4. 프로젝트 요약 생성
        project_summary = create_project_summary(final_code_summary, final_pr_summary, final_commit_summary, githubID, repo_url)

        # 5. 프로젝트 요약을 간단히 변환
        simplified_summary = simplify_project_info(project_summary)
...
def process_code_files(repo_url, githubID):
...
def process_pr_text(repo_url, githubID):
...
def process_commit_diffs(repo_url, githubID, githubName):
...
def create_project_summary(code_summary, pr_summary, commit_summary, githubID, repo_url):
    project_summary = generate_project_summary(code_summary, pr_summary, commit_summary, settings.openai_api_key, prompt=settings.final_project_prompt)
    save_summaries_to_file(project_summary, githubID, repo_url, output_folder=settings.project_data)
    return project_summary

# 프로젝트 요약을 간단한 형태로 변환
def simplify_project_info(project_summary):
